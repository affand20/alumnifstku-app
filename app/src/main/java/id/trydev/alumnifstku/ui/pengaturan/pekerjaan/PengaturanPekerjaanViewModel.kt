            ...
            try {
                when(val response = ApiFactory.createTrace(
                    ...
                    is Result.Error -> {
                        ...
                        _response.postValue(Gson().fromJson(response.exception, DefaultResponse::class.java) as DefaultResponse<Tracing>)
                    }
        ...
        uiScope.launch {
            ...
            try {
                when(val response = ApiFactory.updateTrace(
                    ...
                    is Result.Error -> {
                        _state.postValue(RequestState.REQUEST_ERROR)
                        _responseupdate.postValue(Gson().fromJson(response.exception, DefaultResponse::class.java) as DefaultResponse<Tracing>)
                    }
                }
            }catch (t: Throwable) {
                _state.postValue(RequestState.REQUEST_ERROR)
