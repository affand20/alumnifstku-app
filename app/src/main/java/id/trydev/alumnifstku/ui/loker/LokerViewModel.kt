...
class LokerViewModel: ViewModel() {
    ...
    * */
    private val _response = MutableLiveData<DefaultResponse<List<Loker>>>()
    val response: LiveData<DefaultResponse<List<Loker>>>
        get() = _response
    ...
    * */
    fun getLoker(apiToken: String, query: Map<String, String?>) {
        _state.postValue(RequestState.REQUEST_START)
        uiScope.launch {
            try {
                when(val response = ApiFactory.listLoker(apiToken, query)) {
                    is Result.Success -> {
